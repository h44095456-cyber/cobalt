...
func (cg *CodeGenerator) Generate(program *ast.Program) (string, error) {
	...
	cg.buf.WriteString("#include <X11/Xlib.h>\n")
	cg.buf.WriteString("#include <X11/Xutil.h>\n")
	cg.buf.WriteString("#undef None\n")
	cg.buf.WriteString("#undef Status\n")
	cg.buf.WriteString("#undef Bool\n\n")
	...
	cg.buf.WriteString("// Raylib Native C++ X11 Window Engine\n")
	cg.buf.WriteString("struct CobaltX11Window {\n")
	cg.buf.WriteString("    Display* display = nullptr;\n")
	cg.buf.WriteString("    Window window = 0;\n")
	cg.buf.WriteString("    GC gc = nullptr;\n")
	cg.buf.WriteString("    Pixmap pixmap = 0;\n")
	cg.buf.WriteString("    int width = 800;\n")
	cg.buf.WriteString("    int height = 600;\n")
	cg.buf.WriteString("    bool should_close = false;\n")
	cg.buf.WriteString("    bool keys[512] = {false};\n")
	cg.buf.WriteString("};\n\n")
	cg.buf.WriteString("static CobaltX11Window g_x11;\n\n")
	cg.buf.WriteString("extern \"C\" {\n")
	cg.buf.WriteString("inline void InitWindow(int width, int height, const char* title) {\n")
	cg.buf.WriteString("    g_x11.display = XOpenDisplay(NULL);\n")
	cg.buf.WriteString("    if (!g_x11.display) return;\n")
	cg.buf.WriteString("    int screen = DefaultScreen(g_x11.display);\n")
	cg.buf.WriteString("    g_x11.width = width;\n")
	cg.buf.WriteString("    g_x11.height = height;\n")
	cg.buf.WriteString("    g_x11.window = XCreateSimpleWindow(g_x11.display, RootWindow(g_x11.display, screen),\n")
	cg.buf.WriteString("                                        100, 100, width, height, 1,\n")
	cg.buf.WriteString("                                        BlackPixel(g_x11.display, screen),\n")
	cg.buf.WriteString("                                        BlackPixel(g_x11.display, screen));\n")
	cg.buf.WriteString("    XStoreName(g_x11.display, g_x11.window, title);\n")
	cg.buf.WriteString("    XSelectInput(g_x11.display, g_x11.window, ExposureMask | KeyPressMask | KeyReleaseMask | StructureNotifyMask);\n")
	cg.buf.WriteString("    XMapWindow(g_x11.display, g_x11.window);\n")
	cg.buf.WriteString("    g_x11.gc = XCreateGC(g_x11.display, g_x11.window, 0, NULL);\n")
	cg.buf.WriteString("    g_x11.pixmap = XCreatePixmap(g_x11.display, g_x11.window, width, height, DefaultDepth(g_x11.display, screen));\n")
	cg.buf.WriteString("    XFlush(g_x11.display);\n")
	cg.buf.WriteString("}\n")
	cg.buf.WriteString("inline void CloseWindow() {\n")
	cg.buf.WriteString("    if (g_x11.display) {\n")
	cg.buf.WriteString("        if (g_x11.pixmap) XFreePixmap(g_x11.display, g_x11.pixmap);\n")
	cg.buf.WriteString("        if (g_x11.gc) XFreeGC(g_x11.display, g_x11.gc);\n")
	cg.buf.WriteString("        if (g_x11.window) XDestroyWindow(g_x11.display, g_x11.window);\n")
	cg.buf.WriteString("        XCloseDisplay(g_x11.display);\n")
	cg.buf.WriteString("        g_x11.display = nullptr;\n")
	cg.buf.WriteString("    }\n")
	cg.buf.WriteString("}\n")
	cg.buf.WriteString("inline bool WindowShouldClose() {\n")
	cg.buf.WriteString("    if (!g_x11.display) return true;\n")
	cg.buf.WriteString("    while (XPending(g_x11.display) > 0) {\n")
	cg.buf.WriteString("        XEvent ev;\n")
	cg.buf.WriteString("        XNextEvent(g_x11.display, &ev);\n")
	cg.buf.WriteString("        if (ev.type == DestroyNotify) {\n")
	cg.buf.WriteString("            g_x11.should_close = true;\n")
	cg.buf.WriteString("        } else if (ev.type == KeyPress) {\n")
	cg.buf.WriteString("            KeySym ks = XLookupKeysym(&ev.xkey, 0);\n")
	cg.buf.WriteString("            if (ks < 512) g_x11.keys[ks] = true;\n")
	cg.buf.WriteString("        } else if (ev.type == KeyRelease) {\n")
	cg.buf.WriteString("            KeySym ks = XLookupKeysym(&ev.xkey, 0);\n")
	cg.buf.WriteString("            if (ks < 512) g_x11.keys[ks] = false;\n")
	cg.buf.WriteString("        }\n")
	cg.buf.WriteString("    }\n")
	cg.buf.WriteString("    return g_x11.should_close;\n")
	cg.buf.WriteString("}\n")
	cg.buf.WriteString("inline void BeginDrawing() {\n")
	cg.buf.WriteString("    if (!g_x11.display) return;\n")
	cg.buf.WriteString("    XSetForeground(g_x11.display, g_x11.gc, 0x141821);\n")
	cg.buf.WriteString("    XFillRectangle(g_x11.display, g_x11.pixmap, g_x11.gc, 0, 0, g_x11.width, g_x11.height);\n")
	cg.buf.WriteString("}\n")
	cg.buf.WriteString("inline void EndDrawing() {\n")
	cg.buf.WriteString("    if (!g_x11.display) return;\n")
	cg.buf.WriteString("    XCopyArea(g_x11.display, g_x11.pixmap, g_x11.window, g_x11.gc, 0, 0, g_x11.width, g_x11.height, 0, 0);\n")
	cg.buf.WriteString("    XFlush(g_x11.display);\n")
	cg.buf.WriteString("}\n")
	cg.buf.WriteString("inline void ClearBackground(int color) {\n")
	cg.buf.WriteString("    if (!g_x11.display) return;\n")
	cg.buf.WriteString("    XSetForeground(g_x11.display, g_x11.gc, color);\n")
	cg.buf.WriteString("    XFillRectangle(g_x11.display, g_x11.pixmap, g_x11.gc, 0, 0, g_x11.width, g_x11.height);\n")
	cg.buf.WriteString("}\n")
	cg.buf.WriteString("inline void DrawRectangle(int x, int y, int w, int h, int color) {\n")
	cg.buf.WriteString("    if (!g_x11.display) return;\n")
	cg.buf.WriteString("    XSetForeground(g_x11.display, g_x11.gc, color);\n")
	cg.buf.WriteString("    XFillRectangle(g_x11.display, g_x11.pixmap, g_x11.gc, x, y, w, h);\n")
	cg.buf.WriteString("}\n")
	cg.buf.WriteString("inline void SetTargetFPS(int fps) {}\n")
	cg.buf.WriteString("inline bool IsKeyDown(int key) {\n")
	cg.buf.WriteString("    WindowShouldClose();\n")
	cg.buf.WriteString("    if (key < 512) return g_x11.keys[key];\n")
	cg.buf.WriteString("    return false;\n")
	cg.buf.WriteString("}\n")
	cg.buf.WriteString("inline bool IsKeyPressed(int key) {\n")
	cg.buf.WriteString("    return IsKeyDown(key);\n")
	cg.buf.WriteString("}\n")
	cg.buf.WriteString("}\n\n")
